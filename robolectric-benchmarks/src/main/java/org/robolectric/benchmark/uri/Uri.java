...
package org.robolectric.benchmark.uri;
...
public abstract class Uri implements Comparable<Uri> {
  ...
  private static class StringUri extends AbstractHierarchicalUri {
    ...
    private int findFragmentSeparator() {
      return cachedFsi == NOT_CALCULATED ? cachedFsi = uriString.indexOf('#', findSchemeSeparator())
          : cachedFsi;
    }
    ...
    private String parseSsp() {
      ...
      return fsi == NOT_FOUND ? uriString.substring(ssi + 1) : uriString.substring(ssi + 1, fsi);
    }
    ...
    private PathPart getPathPart() {
      return path == null ? path = PathPart.fromEncoded(parsePath()) : path;
    }
    ...
    private Part getQueryPart() {
      return query == null ? query = Part.fromEncoded(parseQuery()) : query;
    }
    ...
    private Part getFragmentPart() {
      return fragment == null ? fragment = Part.fromEncoded(parseFragment()) : fragment;
    }
    ...
    public Builder buildUpon() {
      if (isHierarchical()) {
        return new Builder().scheme(getScheme())
  ...
  private abstract static class AbstractHierarchicalUri extends Uri {
    ...
    private Part getUserInfoPart() {
      return userInfo == null ? userInfo = Part.fromEncoded(parseUserInfo()) : userInfo;
    }
    ...
    private String parseHost() {
      ...
      String encodedHost = portSeparator == NOT_FOUND ? authority.substring(userInfoSeparator + 1)
    ...
    public int getPort() {
      return port == NOT_CALCULATED ? port = parsePort() : port;
    }
  ...
  private static class HierarchicalUri extends AbstractHierarchicalUri {
    ...
    private HierarchicalUri(String scheme, Part authority, PathPart path, Part query,
        Part fragment) {
      this.scheme = scheme;
    ...
    private Part getSsp() {
      return ssp == null ? ssp = Part.fromEncoded(makeSchemeSpecificPart()) : ssp;
    }
  ...
  public List<String> getQueryParameters(String key) {
    ...
    do {
      ...
      if (separator - start == encodedKey.length() && query.regionMatches(start, encodedKey, 0,
          encodedKey.length())) {
        if (separator == end) {
          values.add("");
        } else {
